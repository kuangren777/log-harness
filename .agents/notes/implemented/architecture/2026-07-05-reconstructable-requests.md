...
- A referenced-text store may declare `mode: 'live'`, a scoped exception to byte-exact reconstruction: resolution follows the store's current text and the logged digest records provenance only; [live referenced-text stores](2026-08-31-live-referenced-text-stores.md) owns that decision.
