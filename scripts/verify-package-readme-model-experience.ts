...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/sci/sci-models': { kind: 'none', reason: 'The catalog registers no prompt, tool, or context; it reaches the user\'s model selector, and its whitelist refusal is an error finish the agent loop raises as an LlmError, so it never enters a model request.' },
  ...
  'packages/client/ui-brand-sci': { kind: 'none', reason: 'Browser-side presentation occupants and theme tokens; registers nothing model-facing.' },
