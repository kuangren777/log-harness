...
The content-addressed text seam. `ctx.referencedText` owns the `referenced-text` content block, the registry of named stores that can produce its text, and the digest verification that proves a logged reference still names the same text.
...
The exact UTF-8 text the store returns, as an ordinary `text` block in the position the `referenced-text` block occupied. The model never sees the store name, the id, or the digest; a reference that fails verification produces an error instead of substitute text.
...
Append-only while the referenced text is stable, because resolution is deterministic: the same reference yields byte-identical request text on every request, preserving an already-reusable prefix. Editing the stored text changes its digest, which makes the old reference fail verification rather than silently rewriting an earlier request position.
