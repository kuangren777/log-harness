...
// ONE register() call declares every child slot + seats the store factory
...
async function bench() {
  ...
  ctx.provide('connection', { api: { settings: {} }, isLoopback: false, configurationPlane: 'memory' } as never)
...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with every child declaration', async () => {
    ...
    // …and declared every child in the ledger.
    expect(slots.spec('sidebar')).toEqual({ kind: 'single', scope: 'root' })
    expect(slots.spec('conversation')).toEqual({ kind: 'single', scope: 'session-maybe' })
    expect(slots.spec('details')).toEqual({ kind: 'single', scope: 'session' })
    expect(slots.spec('rail')).toEqual({ kind: 'single', scope: 'root' })
    expect(slots.spec('view')).toEqual({ kind: 'keyed', scope: 'root' })
  })
