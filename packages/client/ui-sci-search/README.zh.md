...
与线路打交道的只有 `src/client/index.ts` 一处。宿主通过 Typert Remote 命名空间 `sci.literature`（`search`、`recent`、`forget`）作答；注入面把这些信封转成 `src/client/contract.ts` 声明的纯记录与完备结局，所以任何组件都见不到 RPC 错误。四个源全部失败会作为一个错误码抵达，读不出的历史会作为空芯片条抵达，两者都渲染为陈述出来的事实，而不是事件处理里的一次抛出。
...
- **记录类型与 Remote 命名空间面是镜像的，不是 import 来的。** `src/client/contract.ts` 复述了规格 §2.1 的 `LiteratureRecord`/`LiteratureSearchResult` 词汇，`src/client/index.ts` 复述了三个端点，因为在 `@deepseek-ai/dsh-sci-literature` 进入代码树之前，生成的 `sci.literature` 声明并不存在。组装步骤会把两者换成该包的类型与生成的命名空间；届时两边若有偏差会以类型错误的形式暴露，而不是在这里静默地跑错。
