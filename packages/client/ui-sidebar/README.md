...
On a phone the layout renders this shell inside its off-canvas drawer and always passes `collapsed: false`, so the column has no rail state there; the drawer's own toggle is this shell's existing collapse control, which the layout reads as a close. Under a touch-primary pointer the wide-form controls grow to the 44px tap-target floor — the rail's 36px boxes deliberately do not, because a 44px control does not fit a 56px column — and the rail's hover-revealed expand icon becomes unconditional, since hover never arrives there.
