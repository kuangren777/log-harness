...
在手机上，布局把这个外壳渲染在它的离屏抽屉里，并始终传入 `collapsed: false`，因此该列在那里没有控制栏形态；抽屉自带的开关就是本外壳原有的折叠控件，布局把它读作关闭。在以触摸为主的指针下，宽形态控件会增大到 44px 的点按目标下限——控制栏的 36px 方块刻意保持不变，因为 44px 控件放不进 56px 的列——并且控制栏上原本靠 hover 才显示的展开图标变为始终显示，因为那里永远等不到 hover。
