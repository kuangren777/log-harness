...
The Host half registers `ui-onboarding` in the user-settings seam so a document written by an earlier product build stays valid; no shipped step reads or writes it, and the shell itself remains policy-free.
