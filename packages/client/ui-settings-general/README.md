...
At 640px and below the panel takes a phone form: full-bleed at the display's safe-area insets, the 188px navigation rail becomes a horizontal chip scroller above the options, and the close button leaves the flow for the panel's top-right corner. The desktop two-pane layout cannot survive that width — a 188px rail would leave the options column narrower than one settings row. Section registrants are unaffected: they render into the same `settings.section` hole, which simply becomes the full width.
