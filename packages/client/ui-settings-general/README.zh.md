...
在 640px 及以下，面板改用手机形态：在屏幕安全区内满幅铺开，188px 的导航栏变成选项区上方的横向胶囊滚动条，关闭按钮移出文档流、落到面板右上角。桌面双栏布局撑不过这个宽度——188px 的导航栏会让选项列比一行设置项还窄。分区注册方不受影响：它们仍渲染进同一个 `settings.section` 空位，只是这个空位变成了整幅宽度。
