...
面板自身的外框位于这一上下分栏之上。表头从路径读出选中项——扩展名徽标与文件名——大小则来自预览发起的那次读取，因此由运行时直接串流的 Office 文档不显示大小行，而不是给出一个猜测值。分段开关在渲染后的预览与文件源码之间切换；图片、PDF 与不可预览的字节没有源码可读，尚无读取结果的文档同理，此时开关保持禁用。源码视图显示期间预览仍挂载着，因此切回时不会重新读取文件。下载把预览已持有的字节组装成 blob 交给浏览器，并在点击后立即释放 object URL——不需要第二条路由，也不需要第二次授权。这张卡不再提供关闭或宽度控制：关闭归栏自身的表头，栏宽是 frame 的固定份额——在这里重复任何一个都是多余的外框。表头之下，本次会话每产出一个文件就有一枚芯片，按扩展名聚合，于是三轮之前写下的报告一次点击即达，而不必沿树走一遍。
...
预览按文件本身分派。只有 `.univer` 在任何读取发生前就按扩展名路由到 Office 框架——它是 Viewer 经 Gateway 流式打开的 SQLite 容器，状态路由也只接受这一个扩展名。导出格式（`.xlsx`、`.docx`、`.pptx`）改按普通字节读取：预览报出大小、提供下载，并点名 `univer_import` 是在线打开它们的路径——把它们塞进框架只会死在「运行时不可用」提示上，因为状态路由拒绝这些扩展名。其余文件经 `workspace.readFile` 整份读取（超出部署 `readFileMaxBytes` 上限的文件会被拒绝，而不是截断），再按宿主推导的 media type 分派：Markdown 经 `MarkdownText` 渲染为正文，源码与纯文本渲染为带大小行的高亮 `CodeBlock`，图片走 data URL，PDF 用 pdf.js 原生渲染——每一页都是面板自己排版的 canvas 纸张卡（浏览器插件的自带外框和深色底一并消失），选中先渲染前八页，其余页藏在写明数量的手势后面。没有渲染器的字节只说明其大小与不可预览。`workspace.readFile` 的七个错误码各有自己的一句话；未识别的错误码按内部失败呈现，而不是静默消失。
...
`ctx.sciFiles` 是其他插件对本模式唯一的动作：`locate(path)` 与点击树行完全一样地固定该路径——记录下会话此刻的产出状态，因此之后的交付仍能自动定位——随后把详情栏推到前台。无论详情栏是否渲染过，这个固定项都会落地，这也正是本模式的 store 在 `apply` 里创建而非在注册座位上声明的原因：详情栏关闭时点击的芯片不能把固定项弄丢。

本包同时接管详情栏 `tool` 模式的面板体，以 `priority: -10` 注册，从而遮蔽内置的卡片式渲染器；把本插件从组合中去掉即恢复那一个。这个座位只是输出体——拥有它的面板会在其上方以自己的「输入」段落渲染调用参数——因此 sci 的读法是「这是哪次调用」加「回了什么」。工具名词与状态在同一行：运行中带活动秒表，已完成/失败在调用头仍在窗口内时带上从派发到落定的耗时。其下是模型所见的结果原文，调用失败时以错误色描边，超过 20000 字符即截断并声明已截断。这里没有分步时间轴，因为线上承载的是一次调用与一个结果，而不是两者之间的进度。

`/client` 导出的是插件体（`apply`/`inject`）、服务类型 `ISciFiles`、两个产出文件推导（`locatedPath`、`allLocatedPaths`）与 `toolDisplayName`。后三者是有意跨出包边界的，作为客户端导出纪律的一处明示例外：sci conversation 会把同一批产出路径列为产出芯片，并在卡片上为同一批调用命名，任何一张表复制一份都会与这里漂移。组件、分派表、路径运算、store 与 watcher 都留在包内，藏在 slot 注册之后。
...
- **本模式的浏览状态是一个实例，而非每会话一个。**store 在 `apply` 里创建，好让 `locate` 能在首次渲染之前写入，代价是固定项与展开目录集合在会话之间共享。固定项携带它被做出时的产出文件，因此切换到产出不同的会话时，它在首次渲染即被释放；展开目录则确实会跨会话保留。要做到每会话一个实例，服务就得把写入排队到详情栏挂载之后。
- **工具显示名不随语言切换。**`toolDisplayName` 是一张纯名称表而非字典查询，因为它的跨包消费方要在任何 locale 座位之外为卡片命名。于是英文界面下工具名词仍是中文；本包其余每一个字符串都走 `sci-files` 词典。
- **接管 `tool` 面板体就是为所有工具接管。**该座位只容一个占用者，因此本插件在组合中时，内置的终端、读取、diff、搜索与网页卡片都不会出现在详情栏里。按工具定制的 sci 卡片应当落在按名分派的 `tool.call.toolview` 座位上，而本包并未占用它。
