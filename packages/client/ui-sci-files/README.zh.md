...
本包同时接管详情栏 `tool` 模式的面板体，以 `priority: -10` 注册，从而遮蔽内置的卡片式渲染器；把本插件从组合中去掉即恢复那一个。sci 的读法回答的是「问了什么、回了什么」。工具名词与状态在同一行——运行中带活动秒表，已完成/失败在调用头仍在窗口内时带上从派发到落定的耗时——其下是两个原文块：调用参数经美化打印（流被截断导致无法解析时原样呈现原始字符串），以及模型所见的结果原文，调用失败时以错误色描边，超过 20000 字符即截断并声明已截断。这里没有分步时间轴，因为线上承载的是一次调用与一个结果，而不是两者之间的进度。
