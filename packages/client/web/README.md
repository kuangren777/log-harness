...
The shell's mount defaults (src/base.css) size `html`, `body` and `#root` to `100dvh` behind an `@supports` guard, with the percentage height as the fallback. A percentage resolves against the LARGE viewport on mobile browsers — the one with the URL bar retracted — so the frame's bottom row starts underneath browser chrome; the dynamic unit tracks the visible viewport as that chrome and the on-screen keyboard move it. `body` also sets `overscroll-behavior: none`, because the app is a fixed frame with its own scroll regions and a document-level rubber-band would pull-to-refresh the client away mid-turn.
