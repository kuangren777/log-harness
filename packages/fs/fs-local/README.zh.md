...
- **`writeBytes`**：采用与 `writeText` 完全相同的暂存文件发布方式（私有 `0o700` 同级目录、排他 `0o600` 临时文件、fsync、rename），并经过同一把按目标的变更锁，因此同一路径上的字节写入与文本写入不会交错。缺失的父目录会被创建，现有文件的 mode 会保留；字节按原样写入，不做 UTF-8 校验、NUL 拒绝或行尾规范化。载荷超过 `config.maxWriteBytes`（默认 64 MiB，上限为运行时的 `buffer.constants.MAX_LENGTH`）时，在创建暂存目录之前就以 `FS_TOO_LARGE` 拒绝；已存在的非普通文件目标以 `FS_NOT_REGULAR_FILE` 拒绝。它没有版本防护，也不返回结果。
...
- **`writeBytes` 会把完整载荷放在内存中**：该 seam 没有流式写入，因此 `config.maxWriteBytes` 限制的是宿主侧的一次缓冲，而不是允许超大文件分片进入。
