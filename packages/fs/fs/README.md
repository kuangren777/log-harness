...
| `writeBytes(target, data, signal)` | Atomic create/replace from raw bytes, with no decoding, normalization, or diff basis — the write counterpart of `readBytes`, for host plugins that own binary payloads. Unconditional by contract: there is no `FsWriteIntent`, so no `fs/write-intent` decision applies. Creates missing parent directories like `writeText`, refuses a payload past the backend's configured `maxWriteBytes` with `FS_TOO_LARGE` before any content leaves the host, and refuses an existing non-regular target with `FS_NOT_REGULAR_FILE`. |
...
- **Text-only MODEL-FACING mutations** — text reads, `writeText`, and `editText` reject binary/non-UTF-8 content with `FS_NOT_TEXT`; `readBytes`/`writeBytes` are the raw-byte pair, and they are host-plugin primitives with no model-facing tool, so binary tool schemas remain a deliberate deferral of [the tool-schemas Agent Note](../../../.agents/notes/implemented/feature/2026-06-17-filesystem-tool-schemas.md).
- **No guarded raw-byte write** — `writeBytes` takes no `FsWriteIntent` and no per-call sandbox policy, so a caller needing create-if-absent semantics or an escalated mode uses `writeText` or checks `stat` itself; the [raw-byte write Agent Note](../../../.agents/notes/proposed/architecture/2026-08-30-fs-write-bytes.md) records why.
