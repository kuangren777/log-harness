...
`command.*` 与 `skill.*` 领域向客户端暴露宿主命令注册表和 skill（技能）目录。每个方法都通过 `sessionId` 寻址一个会话的 Agent（被服务的会话必有 Agent；`command.*` 经由与 `session.*` 相同的路径恢复冷会话，而 `skill.*` 从会话头解析项目根目录，不触碰 Agent 注册表）。`skill.list` 服务于 composer 的菜单：它返回每一个用户可调用的 skill 及其 `modelInvocable` 标志，让菜单能够标出仅限用户（`disable-model-invocation`）的条目——斜杠手势是这类条目唯一的调用路径。`skill.inventory` 回答的则是策略编辑器的问题：它按来源分组报告每一个被发现的 skill，最近的层与最优 rank 在前，并保留目录会丢弃的被遮蔽落选项；每个条目携带其自述策略、生效策略、二者之间的已存覆盖，以及提供方有文件时的发现路径。若某个提供方失败或目录在发现过程中发生变化，`complete` 为 false，编辑器据此重试，而不是把局部视图当作全貌；该读取不走缓存，既不填充也不驱逐菜单所读的目录缓存。由于它投射已存的 `skills` 设置分节与宿主绝对路径，浏览器载体把 `skill.inventory` 与 `settings.describe` 一并钉入特权集合，而 `skill.list`——不含路径、不含覆盖——对每个客户端保持可达。两者都只读——调用本身就是一次普通的 `session.prompt`，`dsh-tool-skill` 会在 pre-step 边界识别其中以空白为界的 `/name` token，并以注入的 `<skill_content>` 上下文作答，因此所有入口（Web、TUI 与 ACP（Agent Client Protocol））共享同一条确定性路径，手动键入的文本也走该路径，且没有专设的调用协议。`command.execute` 在宿主侧运行一条斜杠命令行，语义为纯准入：响应报告该行是否解析到处理器，并在解析到时回带铸造的生命周期 `commandId`（将本次确认与流节点关联）；结局经由持久落账并在 mux 流广播的 `command/run`/`command/done` 生命周期事件对承载。命令处理器运行超过 30 秒的传输健康时限仍属正常，因此 `command.execute` 仅携带调用方／连接取消信号；该信号可取消正在运行的处理器。`commands/change` 搭乘转发事件帧作为注册表级目录失效信号：客户端重新拉取 `command.list` 而不是做差分。转发的 `agent-preset/selected` 是它按会话粒度的对应物，由落账的选择提交点发出：重组空会话的 agent 只是重新挂接其 scope，不产生任何注册，因此该会话组成所决定的两份目录（`command.list`、`skill.list`）都会失效，却没有任何注册表变化来宣告它。转发的 `skills/change` 则是 skill 注册表自身的不带过滤条件的对应物：提供方、运行时贡献或已存覆盖发生了变化，而它既不点名会话也不点名 skill，因此客户端会丢弃全部已缓存的 `skill.list`／`skill.inventory` 应答并重新拉取。
...
每个方法携带一条策略，写在 `src/fetch/handler.ts` 中紧挨分发表的一张表里，类型为 `{ [K in keyof RpcMethodMap]: PolicyRow<K> }`：新方法在被赋予策略之前无法通过编译，而且就在为它添加路由的同一次改动中。`user` 放行任何已认证调用方，`admin` 要求内置管理员组成员身份，`owner` 要求调用方拥有 payload 所指的每个会话或工作区——`owner` 行还必须指明据以解析归属的 payload 键（`sessionId`、`parentSessionId` 或 `workspaceId`），这正是让这些行可被检查的原因。这里没有默认值：被人遗漏的那个方法，正是绝不能悄悄变得可达的那个。

跨账户作答的方法收窄自己的答案而不是拒绝：`session.list`、`session.search` 与 `workspace.list` 是 `user`，只返回调用方拥有的部分。当没有东西可收窄时，拒绝才是那道强制——subagent 方法是基于 `parentSessionId` 的 `owner`，因为 subagent 目录是某一段父对话的投影，子代自身不携带归属记录。`session.create` 是 `user`，因为会话 id 由客户端预先分配；实现自身会拒绝调用方并不拥有的 id 或工作区，并记录新会话的归属。
