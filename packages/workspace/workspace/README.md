...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` in the filesystem the tools execute in, rejects a path absent there or not a directory, creates at most one record per canonical path, and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title.
...
Every path this package canonicalizes, compares, and stores belongs to one filesystem: the composed `fs` service's execution world when a filesystem service is mounted — `resolve` then `processPath` supply the canon, and a target the backend does not have is that world's missing path — and the Host process filesystem (`realpath` plus `stat`) when no filesystem service is composed. A sandboxed backend runs every tool inside the sandbox, so `/home/user/sci/projects/x` is a sandbox path and not a Host path; canonicalizing it on the Host would reject exactly the directories a sandbox-backed picker hands over. The service is optional and read per call, so `WorkspaceRecord.path` means the canonical path in whichever world stamped it and a record only validates under that same world.
...
- A workspace record does not name the world that stamped its path: recomposing a deployment onto a different filesystem backend (Host to sandbox, or between sandboxes) leaves stored paths unresolvable there, and the registry reports `missing-dir` rather than migrating them.
