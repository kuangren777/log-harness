...
- `ctx.workspaceRegistry.create(path, title?)`：在工具实际执行所在的文件系统里规范化 `path`，拒绝在该文件系统中不存在或非目录的路径，每个规范路径最多创建一条记录，并将新记录前置到持久 workspace 顺序。对同一路径重复调用会返回现有 workspace，且不改变其标题；不同路径可以共用显示标题。
...
本包规范化、比较和存储的每一个路径都属于同一个文件系统：composition 中挂载了 `fs` 服务时，属于该服务后端的执行世界——由 `resolve` 加 `processPath` 给出规范路径，后端没有的目标即该世界中的缺失路径；没有挂载文件系统服务时，属于 Host 进程文件系统（`realpath` 加 `stat`）。沙箱后端会把每个工具都放进沙箱里运行，因此 `/home/user/sci/projects/x` 是沙箱路径而不是 Host 路径；在 Host 上做规范化恰好会拒绝沙箱目录选择器交回的那些目录。该服务是可选的，且每次调用都重新读取，因此 `WorkspaceRecord.path` 指的是当初为它盖章的那个世界里的规范路径，记录也只在同一个世界里才能通过校验。
...
- workspace 记录不会记下为其路径盖章的世界：把部署改挂到另一个文件系统后端上（Host 换沙箱，或沙箱之间互换）后，已存储的路径在那里都无法解析，注册表只会报告 `missing-dir`，不会迁移它们。
