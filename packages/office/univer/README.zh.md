...
| `/assets/<name>`（exact，每个文件一条） | — | Viewer 自己的 chunk，直接由 `artifacts/viewer/assets` 提供，不经代理。 |

HTML 与 CSS 响应中的绝对 `/assets/` 引用会在送达浏览器前被改写为 `/univer-gw/assets/`，因为 harness web app 已经在提供 `/assets/*`，两者会相撞。其他内容类型的响应体原样流式透传。

这条改写只够到标记文本，而 Viewer 的 JavaScript 还会去取标记里从未出现的 chunk：它的 module-preload helper 把依赖存成相对列表（`"assets/en-US-CcPImG2f.js"`），逐个变成 `"/" + dep`，再相对模块 URL 解析——因此无论发起导入的 chunk 是从哪个路径取到的，请求发出去都是绝对的 `/assets/<name>`。为此 `src/host/webServer/viewer-assets.ts` 在插件加载时枚举一次 `artifacts/viewer/assets`，为其中每个文件注册一条 **exact** `/assets/<name>` route，按扩展名给出 content type，并带上 `cache-control: public, max-age=31536000, immutable`——这些文件名由 Viewer 自己的构建做了内容哈希。web server 先查 exact 表再查前缀表，于是这些名字取到内置文件，而其余 `/assets/*` 路径仍归 harness web app；`/assets` 这个**前缀**本身从不被占用。被枚举出的集合就是全部允许清单：exact route 只匹配一个解析后的 pathname，并携带它构建时就确定的文件路径，因此任何请求都无法指向该目录之外的文件。没跑过 `pnpm run fetch-artifacts` 的检出没有这个目录，于是不注册任何 route，并在加载时说明一次。

每一条路由都有防线。`/univer-api`、`/univer-gw`、`/uf`、`/uf` 的 upgrade，以及每一条 `/assets/<name>`，都会先跑该部署的浏览器信任检查——`ctx.connection.isTrustedRequest`，与 RPC 通道对 `/api` 所用的判断完全一致——被拒则回 403。这条防线正是 `connection` 必须注入而非可选的原因：`trustedHosts` 归它所有，在这里再复制一份策略就可能与 `/api` 判断不一致。发往 Gateway 的这一跳从本代理自己掌控的请求头集合出发：`cookie`、`authorization`、`proxy-authorization`、`forwarded` 以及全部 `x-forwarded-*` 都会被扣下，因为 Gateway 是一个无鉴权的 loopback 服务，否则就会拿到操作者的环境凭据和一个由客户端选定的来源地址。回程上 `set-cookie` 会被丢弃，使 Gateway 无法在 harness 源上种 cookie。上游目标一律由**解析后的** pathname 构建，绝不对原始 target 做切片；请求在规范化后离开了前缀，或带有编码后的穿越段，一律以 400 拒绝。
...
- **vendor 进来的源码大部分不受逐文件覆盖率闸门约束。** 豁免清单写在 `vitest.config.ts` 里。有十五个由 harness 编写或修改的文件仍被 100% 闸门约束：`src/index.ts`、`src/invariant.ts`、`src/tools.ts`、`src/client/viewer-url.ts`、`src/client/viewer-locale.ts`、`src/host/config.ts`、`src/host/index.ts`、`src/host/artifacts/paths.ts`、`src/host/provider/plugin.ts`、`src/host/service/univer-service.ts`、`src/host/tools/names.ts`、`src/host/tools/plugin.ts`、`src/host/webServer/gateway-proxy.ts`、`src/host/webServer/plugin.ts` 与 `src/host/webServer/viewer-assets.ts`。其余部分——Gateway HTTP 与 worker 适配器、进程监管、文档操作、工具定义以及浏览器组件——都是锁定的上游副本，由上游自己的 smoke 套件覆盖。其中两个被豁免的文件带有本地改动，但被间接覆盖：`src/client/components/review-panel.tsx` 调用受闸门约束的 `editViewerUrl`，`src/host/provider/gateway-univer-service.ts` 产出受闸门约束的 `GATEWAY_PROXY_PREFIX`。
...
- **Viewer 的 chunk 文件名会遮蔽 harness web app 中同名的文件。** exact 的 `/assets/<name>` route 先于 web app 的 `/assets/` 前缀被解析，因此若某次构建的 dist 里恰好有一个与内置 Viewer chunk 完全同名的文件，返回的将是 Viewer 的字节。双方的文件名都做了内容哈希，这使碰撞不太可能发生，但并非不可能，且没有任何机制会发现它。
