...
Each component is `round_half_up(tokens × micros ÷ 1_000_000)`, the four are summed, and the peak multiplier is applied to that sum with one final half-up rounding. Rounding once per component and once per call keeps the number reproducible from the ledger row alone; multiplying the prices first would compound a rounding error per component.
