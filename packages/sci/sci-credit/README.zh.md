...
每个分量都是 `round_half_up(tokens × micros ÷ 1_000_000)`，四项求和，再把峰谷乘子作用在这个和上做最后一次四舍五入（半数进位）。每分量一次、每次调用一次的舍入，让金额可以只凭账本行复算；先乘价格则会让舍入误差按分量累积。
...
**价目表里没有的模型**按表里最贵的一行计价，并在扣费体与会话记录上都标记 `unknownModel`。比较顺序是输出价、未命中输入价、命中输入价、模型 id，所以选择与表的顺序无关。往贵的方向错是安全的方向：另一种做法是免费提供一个没定价的模型，直到有人发现账本不对。
