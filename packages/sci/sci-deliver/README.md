...
- **A non-UTF-8 snapshot is stored base64-encoded.** `ctx.fs` exposes `writeText` only — no binary write, no copy — so a delivered PNG or PDF is snapshotted as base64 text under a `.base64` suffix. The `sha256` and `size` in the event always describe the original bytes, so a card projected from the event is correct; a consumer reading the snapshot file must honour the suffix. A binary write verb on the filesystem seam would remove the encoding entirely.
