...
`AuditFold` holds the little that one event cannot decide: two relations, both anchored on the most recent `sci/plan-declared`. A workflow run belongs to the plan declared before it (`tool-workflow/run-start` names only the run), so the fold attaches the run id to the open declaration, once. And the agents the fan-outs actually start belong to that declaration too: every `subagent_<persona>` call and every `tool-workflow/agent-start` after it, until the next declaration, re-emits the `sci_plan` row with `spawnedAgents`, `spawnedPersonasJson` (a workflow agent is recorded as `workflow:<label>`), and `reconciled` — `fewer`, `match`, or `more` against `declaredAgents`. The studied platform drew a plan card and ran whatever its script did, and nothing compared the two (`clawsgo-analysis/CLAWSGO-SCHEDULING.md` §5 row 8); the row is that comparison, and `planRecords` returns the settled record of every declaration in a log. `projectLog` runs a fresh fold over a whole log and is what `rebuild` replays.
...
`summarize(sessionId)` returns the refusal count (`tool-denied` plus `fs-denied` rows), the delivery count, the count of explicitly granted authorizations, the memory write-timing score when the memory index is composed, whether the session missed a citation, `planMismatches` (declarations whose started agents never matched their roster), and `deliveriesWithoutExecution` (deliveries made before any `execToolNames` call returned in the session — a deliverable that no execution preceded is the shape of the studied platform's fabricated reproduction, §3 of the analysis; measured, not gated). The counts come from the committed rows rather than from the log, so a caller sees what the projection actually committed; a divergence between the two is what `rebuild` exists to expose.
...
- **Provenance is measured at the session, not at the number.** `deliveriesWithoutExecution` says whether any execution returned before a delivery; it does not tie a figure inside a deliverable to the `tool/result` that produced it. A finer projection needs a definition of "a number in a deliverable" first (`clawsgo-analysis/CLAWSGO-SCHEDULING.md` §5 row 4); until then the mandatory adversary in `@deepseek-ai/dsh-sci-plan` is the line against a fabricated result, and this figure is the audit-side signal.
- **Retrieval redundancy is a floor.** `@deepseek-ai/dsh-sci-agents` counts a repeat only when a child calls the same web tool with the same arguments text; a re-worded query over the same paper set is not detected (§5 row 9).
