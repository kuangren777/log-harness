...
于是 `sci-prompt`、`sci-skills`、`sci-workspace`、`sci-guard`、`sci-credit`、`sci-memory`、`sci-audit`、`sci-remote-hosts`、`sci-tier/fork` 在 `cordis.patch.yml` 里，而 `sci-tier`、`sci-tier/suggest`、`sci-plan`、`sci-deliver` 和委派工具在 preset 里。`sci-tier/fork` 归 host plane 的理由与 subagent 注册表相同：包入口是一个**两个 preset 都挂**的函数插件，从那里发布服务会在第二个 preset 挂载的一刻撞名。`sci-credit` 归 host plane 是因为它为进程里每个 agent 计量同一条 `llm/stream` waterfall；它的 `vmToken` 没有默认值：没有 gate 的部署应当删掉那一行，而不是把 token 留空。
