...
| Auto preset, shown as `自动 / Auto` | `config/agent-presets/sci-auto/` | the same roster |
...
| Persona roster plugin | `src/index.ts` | the `sci-cluster` and `sci-auto` presets |
...
That puts `sci-prompt`, `sci-skills`, `sci-workspace`, `sci-guard`, `sci-credit`, `sci-memory`, `sci-audit`, `sci-remote-hosts`, `sci-tier/fork`, and `office-univer` in `cordis.patch.yml`, and `sci-tier`, `sci-tier/suggest`, `sci-plan`, `sci-deliver`, `office-univer/tools`, and the delegation tools in the presets, with `camel-runtime` (persistent project variants over `ctx.e2b` and AgentENV) in the cluster preset only, its row gated on `AENV_API_KEY` so a deployment without an AgentENV server mounts no variant tools rather than failing every cluster session at load, and its per-workspace cap read from `AENV_MAX_VARIANTS` so the gate can set it per plan. `office-univer` follows the split inside one package: the host row runs the Univer Gateway, publishes the `univer` service, and serves the Viewer with `tools: false` and `skills: false` — the `univer_*` skills are the protected built-in tier the skill vault serves through `sci-skills`, so the package's bundled copies stay unpublished — and each preset mounts `@deepseek-ai/dsh-office-univer/tools` over that service, withholding `univer_screenshot` and `univer_lint` because the dsh image ships no headless Chromium. `sci-tier/fork` is host-plane for the same reason as the subagent registry: the package entry is a function plugin BOTH presets mount, and a service published from there would collide the moment the second preset mounted it. `sci-credit` is host-plane because it meters the `llm/stream` waterfall for every agent in the process, and its `vmToken` has no default: a deployment with no gate deletes that row rather than blanking the token.
...
`sci-auto` is `sci-cluster` with the tier row at `auto` and `@deepseek-ai/dsh-sci-tier/resolve` mounting `resolve_tier`: the swarm is composed but shut until the model resolves the tier from the task, and a balanced resolution can be raised mid-session — the task-driven tier the studied platform lacked (`clawsgo-analysis/CLAWSGO-SCHEDULING.md` §6.1). The default preset stays `sci-balanced`; a deployment that wants the task to decide sets `agent-presets.default: sci-auto`.
...
- **The presets restate their shared rows rather than including them.** `dsh-agent-presets` discovers whole directories, so `sci-cluster` repeats every row `sci-balanced` has, and `sci-auto` repeats every row `sci-cluster` has. A row changed in one and forgotten in the other is caught only by the composition tests here, not by the loader.
