...
[`@deepseek-ai/dsh-fs`](../../fs/fs/README.zh.md) 提供方约定的 E2B 实现。它只有一个配置项 `maxWriteBytes`（默认 64 MiB），用于限制单次 `writeBytes` 的载荷。先挂载一个 [E2B 沙箱能力缝](../e2b/README.zh.md)的 provider（提供方）——[`dsh-e2b-cloud`](../e2b-cloud/README.zh.md) 或 [`dsh-dormice`](../dormice/README.zh.md)——再用本服务取代 `dsh-fs-local`。该提供方使用所有者的远程 cwd 和 SDK 句柄，因此文件工具观察到的环境与 E2B 后端 Bash 进程相同。
...
- **有界原始字节写入**：`writeBytes` 在打开暂存目录之前就以 `FS_TOO_LARGE` 拒绝超过 `config.maxWriteBytes` 的载荷，因此超限缓冲绝不会到达控制器。载荷作为二进制体经 envd 的文件 API 上传——与 `writeText` 使用的是同一个请求——因此不需要 base64 shell 往返，内容本身也不占用任何 `commands.run` 槽位；只有周边的 `chmod` 步骤会占用槽位。它与 `writeText`/`editText` 共享按目标的锁，不携带版本防护，也不返回任何内容。
...
- **仍需承担完整文件变更成本**：覆盖差异和字面量编辑会把完整文件读入宿主内存，`writeBytes` 会在一次请求中上传完整载荷（该 seam 没有流式写入），每项操作也都会产生 E2B 控制器延迟。
