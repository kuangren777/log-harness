...
E2B implementation of the [`@deepseek-ai/dsh-fs`](../../fs/fs/README.md) provider contract. Its one config key is `maxWriteBytes` (default 64 MiB), the cap on one `writeBytes` payload. Mount a provider of the [E2B sandbox seam](../e2b/README.md) — [`dsh-e2b-cloud`](../e2b-cloud/README.md) or [`dsh-dormice`](../dormice/README.md) — first, then this service in place of `dsh-fs-local`. The provider uses the owner's remote cwd and SDK handle, so file tools observe the same world as E2B-backed Bash processes.
...
- **Bounded raw-byte writes** — `writeBytes` refuses a payload above `config.maxWriteBytes` with `FS_TOO_LARGE` before opening a staging directory, so an oversized buffer never reaches the controller. The payload goes up as a binary body through envd's file API — the same request `writeText` uses — so no base64 shell round trip is needed and no `commands.run` slot is spent on the content; only the surrounding `chmod` steps take slots. It shares the per-target lock with `writeText`/`editText`, carries no version guard, and returns nothing.
...
- **Whole-file mutation costs remain** — overwrite diffs and literal edits read complete files into host memory, `writeBytes` uploads its complete payload in one request (there is no streamed write on the seam), and every operation incurs E2B controller latency.
