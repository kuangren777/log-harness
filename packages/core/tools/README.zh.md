...
- `ctx.tools.restrictableNames(scope?: ScopeKey): readonly string[]`：该作用域的 `restrict` 过滤器可以指名的全局名字，即它继承到的全部内容，位于其自身注册与任何已生效掩码之前。从策略计算白名单的调用方需要的正是这个集合 —— `schemas()` 是错误的来源，因为它已应用掩码，并且携带作用域本地名字与保留传输名，而这些都是 `restrict` 拒绝接受的。
