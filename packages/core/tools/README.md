...
- `ctx.tools.restrictableNames(scope?: ScopeKey): readonly string[]` the global names a `restrict` filter for that scope may name: everything the scope inherits, before its own registrations and before any mask already in force. A caller computing an allowlist from a policy needs exactly this set — `schemas()` is the wrong source, since it already has masks applied and carries scope-local names and the reserved transport, all of which `restrict` refuses.
