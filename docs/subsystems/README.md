...
| [sci.md](sci.md) | the science-research product layer: `ctx.sciAudit` projection rows, `ctx.sciMemory` recall contracts, `ctx.sciRemoteHosts` managed-block requests, `ctx.sciTierFork` |
