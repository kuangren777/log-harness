...
| [auth.md](auth.md) | the authentication and authorization seam: `Principal`, permission rules and `evaluate`/`permits` precedence, accounts, groups, one-time secrets, audit records, and the `ctx.authGate` request gate |
| [mail.md](mail.md) | the outbound mail seam: `MailMessage`, the single `send` operation, and provider-owned sender identity |
